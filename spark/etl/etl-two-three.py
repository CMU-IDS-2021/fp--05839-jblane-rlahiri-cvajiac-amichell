...
if __name__ == "__main__":
    # Input validation
    if len(sys.argv) != 5:
        print("usage: etl-two-three.py <input_books_list>.txt <books_base_path> <output_path> <stop_words_file>")
    # Get the arguments
    ...
    conf = pyspark.SparkConf().setAppName("ETL-Optimizations-Two-Three")
    ...
    infrequent_words = frequencies\
        .filter(lambda x: x[1][1] < book_lower_limit)\
        .map(lambda x: x[0])
    frequent_words = frequencies\
        .filter(lambda x: x[1][1] > book_upper_limit)\
        .map(lambda x: x[0])
